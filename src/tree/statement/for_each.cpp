...
void mcc::ForEachStatement::Generate(
        Builder &builder,
        Frame &frame) const
{
    ...
    const auto first_element = ElementReference::
            Create(Where, mutable_iterable, ConstantNumber::Create(Where, builder.GetContext(), 0));
