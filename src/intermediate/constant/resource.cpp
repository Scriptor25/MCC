...
mcc::ConstantPtr mcc::ConstantResource::Create(const SourceLocation &where, TypeContext &context, const ResourceLocation &location)
{
...
mcc::ConstantResource::ConstantResource(const SourceLocation &where, TypeContext &context, ResourceLocation location)
    : Constant(where, context.GetNull()), Location(std::move(location))
{
}
