...
mcc::InstructionPtr mcc::SwitchInstruction::Create(const SourceLocation &where, TypeContext &context, const ResourceLocation &location, const ValuePtr &condition, const BlockPtr &default_target, const std::vector<std::pair<ConstantPtr, BlockPtr>> &case_targets)
{
    return std::make_shared<SwitchInstruction>(where, context, location, condition, default_target, case_targets);
}

mcc::SwitchInstruction::SwitchInstruction(const SourceLocation &where, TypeContext &context, ResourceLocation location, ValuePtr condition, BlockPtr default_target, const std::vector<std::pair<ConstantPtr, BlockPtr>> &case_targets)
    : Instruction(where, context.GetVoid(), false), Location(std::move(location)), Condition(std::move(condition)), DefaultTarget(std::move(default_target)), CaseTargets(case_targets)
{
    Condition->Use();
    DefaultTarget->Use();
...
void mcc::SwitchInstruction::Generate(CommandVector &commands, bool stack) const
{
    ...
    case ResultType_Value:
        ...
        if (take_default)
            commands.Append("{}return run function {}{}", prefix, DefaultTarget->Parent->GetLocation(DefaultTarget), arguments);
        break;
    ...
    case ResultType_Reference:
        for (auto &[case_, target_] : CaseTargets)
        {
            ...
            commands.Append("execute if score %c {} matches {} run data modify storage {} {} set value 1",
                            tmp_name,
                            case_value.Value,
                            Location,
                            stack_path);
            ...
            commands.Append("{}execute if data storage {} {} run return run function {}{}",
                            prefix,
                            Location,
                            stack_path,
