...
mcc::InstructionPtr mcc::AllocationInstruction::Create(const SourceLocation &where, const TypePtr &type, const ResourceLocation &location, const IndexT index, const bool is_mutable, const ConstantPtr &initializer)
{
    return std::make_shared<AllocationInstruction>(where, type, location, index, is_mutable, initializer);
}

mcc::AllocationInstruction::AllocationInstruction(const SourceLocation &where, const TypePtr &type, ResourceLocation location, const IndexT index, const bool is_mutable, ConstantPtr initializer)
    : Instruction(where, type, is_mutable), Location(std::move(location)), Index(index), Initializer(std::move(initializer))
{
}
...
void mcc::AllocationInstruction::Generate(CommandVector &commands, const bool stack) const
{
    ...
    Assert(initializer_value.Type == ResultType_Value,
           Where,
