...
static void generate_macro_print(const mcc::MacroInstruction &self, mcc::CommandVector &commands)
{
    ...
    case mcc::ResultType_Reference:
        message_value = std::format("{{\"{}\":\"{}\",\"nbt\":\"{}\",\"interpret\":true}}",
...
static void generate_macro_swap(const mcc::MacroInstruction &self, mcc::CommandVector &commands)
{
    ...
    mcc::Assert(value1.Type == mcc::ResultType_Reference,
                self.Where,
                "first value must be {}, but is {}",
                mcc::ResultType_Reference,
                value1.Type);
    mcc::Assert(value2.Type == mcc::ResultType_Reference,
                self.Where,
    ...
    commands.Append("{}data modify {} {} {} set from {} {} {}",
                    ...
                    value2.Path);
    commands
        .Append("{}data modify {} {} {} set from storage {} {}", prefix3, value2.ReferenceType, value2.Target, value2.Path, self.Location, tmp_name);
...
static void generate_macro_data(const mcc::MacroInstruction &self, mcc::CommandVector &commands)
{
    ...
    mcc::Assert(dst_value.Type == mcc::ResultType_Reference,
                self.Where,
                "dst must be {}, but is {}",
                mcc::ResultType_Reference,
                dst_value.Type);
...
static void generate_macro_store(const mcc::MacroInstruction &self, mcc::CommandVector &commands)
{
    ...
    commands.Append("{}execute store result {} {} {} double 1 run {}",
                    prefix,
                    dst_value.ReferenceType,
                    dst_value.Target,
                    dst_value.Path,
...
mcc::InstructionPtr mcc::MacroInstruction::Create(const SourceLocation &where, TypeContext &context, const ResourceLocation &location, const std::string &name, const std::vector<ValuePtr> &arguments)
{
    return std::make_shared<MacroInstruction>(where, context, location, name, arguments);
}

mcc::MacroInstruction::MacroInstruction(const SourceLocation &where, TypeContext &context, ResourceLocation location, std::string name, const std::vector<ValuePtr> &arguments)
    : Instruction(where, context.GetVoid(), false), Location(std::move(location)), Name(std::move(name)), Arguments(arguments)
{
